...
All of these quantities are then normalized to their value at the sonic radius, denoted with a star ($\rho_0 \rightarrow \rho_0/\rho_\star$), to get dimensionless quantities of order 1. And the set of equations for the normalized quantities is derived. (in [[normalized_zeroth_order_eqs.pdf]]) The final set of differential equations (dropping the index 0) is (eq. 16 - 19 in [[Parks-1978]])
$$
\begin{align}
